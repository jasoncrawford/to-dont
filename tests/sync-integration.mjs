 ...
 * Run with: node tests/sync-integration.mjs
...
async function runTests() {
  ...
  await test('Debug endpoint returns env status', async () => {
    const data = await apiRequest('/api/debug');
  ...
  await test('GET /api/items returns array', async () => {
    ...
    console.log(`  (Found ${data.length} existing items)`);
  });
  ...
  await test('PATCH /api/items/:id updates item', async () => {
    const data = await apiRequest(`/api/items/${testId}`, {
      method: 'PATCH',
